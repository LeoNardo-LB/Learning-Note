...
假设在时间线上，线程A先执行方法`writer`方法，线程B后执行`reader`方法。那执行流程如下图所示：

![volatile内存示意图](_images/volatile内存示意图.jpg)

>   如果`flag`变量**没有**用`volatile`修饰，在step 2，线程A的本地内存里面的变量就不会立即更新到主内存，那随后线程B也同样不会去主内存拿最新的值，仍然使用线程B本地内存缓存的变量的值 `a = 0，flag = false`
...
![image-20200922193455803](_images/image-20200922193455803.png)
